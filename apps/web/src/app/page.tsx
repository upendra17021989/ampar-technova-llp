...
export default function HomePage() {
  return (
    <main id="main-content" className="editorial-home">
      ...
      <section className="section quality-story" aria-labelledby="quality-heading"><div className="shell quality-layout"><div className="quality-collage"><figure className="quality-image quality-image-main"><Image src="/products/frp-piping-installation.jpeg" alt="Corrosion-resistant piping installation" fill sizes="(max-width: 832px) 100vw, 48vw" /></figure><figure className="quality-image quality-image-detail"><Image src="/products/frp-packed-bed-scrubber.jpeg" alt="Fabricated packed-bed scrubber" fill sizes="(max-width: 832px) 48vw, 22vw" /></figure><span className="quality-tag">Built. Checked. Documented.</span></div><div className="quality-copy"><p className="eyebrow dark">Manufacturing evidence</p><TypewriterHeading as="h2" id="quality-heading" text="Quality is built into every stage—not inspected in at the end." /><p>Checks follow the material and fabrication route selected for each application. The result is traceable, reviewable evidence aligned with the approved project requirements.</p><div className="quality-evidence">{qualityEvidence.map((item, index) => <span key={item}><b>0{index + 1}</b>{item}</span>)}</div><Link className="text-link" href="/quality">See our quality approach <span aria-hidden="true">→</span></Link></div></div></section>
