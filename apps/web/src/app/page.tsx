...
export default function HomePage() {
  return (
    <main id="main-content" className="editorial-home">
      ...
      <section className="section technology-story" id="technologies" aria-labelledby="technologies-heading"><div className="shell"><div className="section-heading home-section-heading"><div><p className="eyebrow dark">Material intelligence</p><h2 id="technologies-heading">Three technologies.<br />Selected for the duty.</h2></div><p className="section-intro">Each construction method solves a different balance of corrosion resistance, purity, strength and lifecycle value.</p></div><div className="technology-grid">{technologies.map((technology, index) => <article key={technology.slug}><div className="technology-media"><Image src={technologyImages[index]} alt="" fill sizes="(max-width: 832px) 100vw, 33vw" /></div><span>0{index + 1}</span><h3>{technology.name}</h3><p>{technology.summary}</p><ul>{technology.materials.slice(0, 4).map((material) => <li key={material}>{material}</li>)}</ul><Link className="text-link" href={`/technologies/${technology.slug}`}>Explore technology <span aria-hidden="true">→</span></Link></article>)}</div></div></section>

      <section className="section process-story" id="process" aria-labelledby="process-heading"><div className="shell process-layout"><div className="process-intro"><p className="eyebrow">From design to delivery</p><h2 id="process-heading">A controlled path from process data to site.</h2><p>Every project moves through visible engineering and quality gates.</p><Link className="button button-inverse" href="/capabilities">Explore capabilities</Link></div><ol className="process-steps">{processSteps.map(([number, title, description]) => <li key={number}><span>{number}</span><div><h3>{title}</h3><p>{description}</p></div></li>)}</ol></div></section>
      ...
      <section className="industries-story" aria-labelledby="industries-heading"><div className="shell industries-title"><p className="eyebrow">Industries served</p><h2 id="industries-heading">Where corrosion cannot be an afterthought.</h2></div><div className="industry-marquee" aria-label="Industries served">{industries.slice(0, 8).map((industry, index) => <Link href="/industries" key={industry.slug}><span>0{index + 1}</span>{industry.name}</Link>)}</div></section>
