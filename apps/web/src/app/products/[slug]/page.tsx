...
export default async function ProductDetailPage({ params }: Props) {
  ...
  return <main id="main-content"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} /><section className="page-hero"><div className="shell narrow"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/products">Products</Link><span aria-hidden="true">/</span><span>{product.name}</span></nav><p className="eyebrow">{product.category}</p><h1>{product.name}</h1><p className="lead">{product.summary}</p><div className="action-row"><Link className="button button-primary" href={`/request-a-quote?product=${product.slug}`}>Request a Quote</Link><Link className="button button-inverse" href="/capabilities">View Capabilities</Link></div></div></section><section className="section"><div className="shell detail-grid"><div><p className="eyebrow dark">Typical duties</p><h2>Applications</h2><ul className="feature-list">{product.applications.map((application) => <li key={application}>{application}</li>)}</ul></div><aside className="spec-panel"><h2>Material options</h2><div className="tag-list">{product.materials.map((material) => <span key={material}>{material}</span>)}</div><p className="technical-note">Final material selection depends on chemical, concentration, temperature, pressure and process conditions. Technical review is required.</p></aside></div></section><section className="section section-muted"><div className="shell"><p className="eyebrow dark">Performance</p><h2>Why this equipment is considered</h2><div className="four-grid">{product.benefits.map((benefit) => <article className="card" key={benefit}><h3>{benefit}</h3></article>)}</div>{product.capabilities ? <><h2 className="subsection-title">Available configurations</h2><div className="industry-list">{product.capabilities.map((capability) => <span key={capability}>{capability}</span>)}</div></> : null}</div></section><section className="section cta-section"><div className="shell cta-inner"><div><p className="eyebrow dark">Application review</p><h2>Share your process conditions.</h2><p>Chemical, concentration, temperature, pressure, capacity and site conditions help determine the appropriate construction.</p></div><Link className="button button-primary" href={`/request-a-quote?product=${product.slug}`}>Request This Equipment</Link></div></section></main>;
}
